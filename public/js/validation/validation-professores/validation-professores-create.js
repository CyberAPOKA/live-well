...
jQuery.validator.addMethod("uniqueMatriculaValidation", function(value, element){
    ...
    return valor;
}, "Matricula já registrada no sistema.")
...
jQuery.validator.addMethod("nameValidation", function(value, element){
    regex = new RegExp('^[A-Za-záàâãéèêíïóôõöúçñÁÀÂÃÉÈÍÏÓÔÕÖÚÇÑ1234567890]+$');
    return regex.test($('#name').val());
}, "O nome de usuário não pode ter caracteres inválidos")

jQuery.validator.addMethod("uniqueNomeUsuarioValidation", function(value, element){
    var nomeUsuario = $('#name').val();
    var valor;
    $.ajax({
        async: false,
        type: "GET",
        url:'/nomeusuario/ajax',
        dataType: 'json',
    ...
    return valor;
}, "O nome de usuário já está registrado no sistema.")
...
$(document).ready(function(){
    ...
    $("#formulario").validate({
        rules: {
            ...
            name: {
                required: true ,
                uniqueNomeUsuarioValidation: true,
                nameValidation: true,
            },
            password: {
                required: true,
